...
func NewAuthenticated(broker *auth.Broker, secureCookies bool) (http.Handler, error) {
	...
	mux.HandleFunc("GET /metadata/new", server.metadataNew)
	mux.HandleFunc("POST /metadata/create", server.metadataCreate)
	mux.HandleFunc("GET /metadata/edit", server.metadataEdit)
	mux.HandleFunc("POST /metadata/edit", server.metadataSave)
	mux.HandleFunc("POST /metadata/delete/plan", server.metadataDeletePlan)
	mux.HandleFunc("POST /metadata/delete/apply", server.metadataDeleteApply)
	...
	mux.HandleFunc("GET /workflows", server.workflowList)
	mux.HandleFunc("POST /workflows/mutate", server.workflowMutate)
	mux.HandleFunc("GET /quota", server.quotaList)
	mux.HandleFunc("POST /quota/publish", server.quotaPublish)
	mux.HandleFunc("GET /audit", server.auditList)
